    ...
    def __getitem__(self, index_tuple):
        ...
        for key in blobs:
            if cfg.MODEL.LR_VIEW_ON or cfg.MODEL.GIF_ON or cfg.MODEL.LRASY_MAHA_ON:
                ...
                if key != 'roidb' and key != 'z_position' and key != 'GTbox' :
                    blobs[key] = blobs[key].squeeze(axis=0)
...
def collate_minibatch(list_of_blobs):
    ...
    for i in range(0, len(list_of_blobs), cfg.TRAIN.IMS_PER_BATCH):
        ...
        if cfg.LESION.USE_3D_INPUT:
            mini_list = pad_3d_data(mini_list)
        else:
...
def pad_image_data(list_of_blobs):
    ...
    for blobs in list_of_blobs:
        ...
        if cfg.MODEL.LR_VIEW_ON or cfg.MODEL.GIF_ON or cfg.MODEL.LRASY_MAHA_ON:
            data_padded = np.zeros((2, 3, max_shape[0], max_shape[1]), dtype=np.float32)
            data_padded[:, : , :h, :w] = blobs['data']
        else:
            data_padded = np.zeros((c, max_shape[0], max_shape[1]), dtype=np.float32)
            data_padded[:c, :h, :w] = blobs['data']
        blobs['data'] = data_padded
        output_list.append(blobs)
    return output_list
def pad_3d_data(list_of_blobs):
    # For the case: network are 3Dconv body +2D FPN ,and SLICE_NUM < 32 
    # Do not need depth pad.

    max_shape = blob_utils.get_3d_max_shape([blobs['data'].shape[-3:] for blobs in list_of_blobs])
    output_list = []
    for blobs in list_of_blobs:
        # for 3d data,input shape=C,D,H,W ,where C=1.
        # Depth axis not padded.
        data_padded = np.zeros((1, blobs['data'].shape[1], max_shape[1], max_shape[2]), dtype=np.float32)
        _, d, h, w = blobs['data'].shape
        data_padded[:, :d, :h, :w] = blobs['data']
        blobs['data'] = data_padded
        output_list.append(blobs)
    return output_list
