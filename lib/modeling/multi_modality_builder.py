...
class MULTI_MODALITY_RCNN(Generalized_RCNN):
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        n,c,h,w = data.shape
        im_data = data.view(n*M,3,h,w)#n：batch M: 上下几层。
        input_img=im_data[0,1,:,:].detach().cpu().numpy()
        min_val=input_img.min()
        max_val=input_img.max()
        range_val=max_val-min_val
        input_img=(input_img-min_val)/range_val*255
        if self.training:
            plt.imsave(r'../input_img.png', input_img.astype(np.uint8),cmap='gray')
            roidb = list(map(lambda x: blob_utils.deserialize(x)[0], roidb))

        device_id = im_data.get_device()
        return_dict = {}  # A dict to collect return variables
        if cfg.LESION.USE_POSITION:
            blob_conv, res5_feat = self.Conv_Body(im_data)# Conv_Body resnet+ FPN
        else:
        ...
        if cfg.LESION.CONCAT_BEFORE_RPN:
            for blob in blob_conv:
                _,c,h,w = blob.shape
                blob = blob.view(n, M*c, h, w) #因为之前是把Batch和图数，变到一个维度上了，这里view回去就行
        ...
        #本来的roidb本来是只有GT box的，在这里加进去了很多RPN算出来的ROI，所以既有GT 又有ROI
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            if cfg.LESION.USE_POSITION:
                if cfg.LESION.NUM_IMAGES_3DCE == 3:
                    ...
                    # position_feat = position_feat.view(-1,3,256,h,w)
