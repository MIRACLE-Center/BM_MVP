...
# Lowest and highest pyramid levels in the backbone network. For FPN, we assume # that all networks have 5 spatial reductions, each by a factor of 2. Level 1 # would correspond to the input image, hence it does not make sense to use it.
...
class fpn(nn.Module):
        ...
        for i in range(self.num_backbone_stages):
            if cfg.FPN.USE_GN:
                self.posthoc_modules.append(nn.Sequential(
                    nn.Conv3d(fpn_dim, fpn_dim, 3, 1, 1, bias=False),
    ...
    def detectron_weight_mapping(self):
        print('detectron_weight_mapping #warning')
    ...
    def forward(self, x):
        # self.conv_body is an instance of ResNet.ResNet_convX_body class, which has its own forward method,
        # but here we don't use its own forward method
        if cfg.LESION.DEBUG:
            print('before conv 0,data shape:', x.shape)
        conv_body_blobs = [self.conv_body.res1(x)]
        if cfg.LESION.DEBUG:
            print('conv body res 1 shape:',conv_body_blobs[0].shape)
        for i in range(1, self.conv_body.convX):
            conv_body_blobs.append(
                getattr(self.conv_body, 'res%d' % (i+1))(conv_body_blobs[-1])
            )
        if cfg.LESION.DEBUG:
            for i,blob in enumerate(conv_body_blobs):
                print('conv body ', i, '.shape: ',blob.shape)
        ...
        if cfg.LESION.DEBUG:
            for i,blob in enumerate(fpn_inner_blobs):
                print('fpn inner body ', i, '.shape: ',blob.shape)
        fpn_output_blobs = []
        # import pdb
        # pdb.set_trace()
        for i in range(self.num_backbone_stages):
            x = self.posthoc_modules[i](fpn_inner_blobs[i])
            #n, c, d, h ,w = x.shape
            #max_x = x[:,:, cfg.LESION.SLICE_NUM // 2, :,:]
            #max_x = x.view(n, -1, h, w)
            fpn_output_blobs.append(x)
        if cfg.LESION.DEBUG:
            for i,blob in enumerate(fpn_output_blobs):
                print('fpn output ', i, '.shape: ',blob.shape)
...
class topdown_lateral_module(nn.Module):
    ...
    def __init__(self, dim_in_top, dim_in_lateral):
        ...
        if cfg.FPN.USE_GN:
            self.conv_lateral = nn.Sequential(
                nn.Conv3d(dim_in_lateral, self.dim_out, 1, 1, 0, bias=False),
                nn.GroupNorm(net_utils.get_group_gn(self.dim_out), self.dim_out,
                             eps=cfg.GROUP_NORM.EPSILON)
            )
        else:
            self.conv_lateral = nn.Conv3d(dim_in_lateral, self.dim_out, 1, 1, 0)
    ...
    def forward(self, top_blob, lateral_blob):
        ...
        if cfg.LESION.DEBUG:
            print('td:',td.shape)
            print('lat: ',lat.shape)
            print('topblob: ',top_blob.shape,'\tlateral_blob:',lateral_blob.shape)
        return lat + td
...
FpnLevelInfo = collections.namedtuple(
    'FpnLevelInfo',
    ['blobs', 'dims', 'spatial_scales', 'slice_index']
)
