...
import modeling.myResNet as myResNet
import modeling.ResNetBN as ResNetBN
...
def fpn_ResNet26_conv5_body():
    return fpn(
        ResNet.ResNet26_conv5_body, fpn_level_info_ResNet50_conv5()
    )

def fpn_myResNet50_conv5_body():
    return fpn(
        myResNet.ResNet50_conv5_body, fpn_level_info_ResNet50_conv5()
    )
...
def fpn_ResNet50_conv5_small_body():
    return fpn(
        ResNet.ResNet50_conv5_small_body, fpn_level_info_ResNet50_conv5()
    )

def fpn_ResNetBN50_conv5_body():
    return fpn(
        ResNetBN.ResNet50_conv5_body, fpn_level_info_ResNet50_conv5()
    )

def fpn_ResNet50_conv5_body_cabu():
    return fpn(
        ResNet.ResNet50_conv5_body, fpn_level_info_ResNet50_conv5(cascade_bu=True)
    )
...
class fpn(nn.Module):
    ...
    def forward(self, x):
        # position at res1
        if cfg.LESION.SHALLOW_POSITION:
            res1_feat = self.conv_body.res1(x)
            position_feat = self.Position_Head(res1_feat)
            pos_cls_pred = self.Position_Cls_Outs(position_feat)
            pos_reg_pred = self.Position_Reg_Outs(position_feat)
            #conv_body_blobs = [res1_feat]
            # pos se
            b,c,h,w = res1_feat.size()
            y = self.pos_fc(position_feat).view(b,c,1,1)
            conv_body_blobs = [res1_feat*y.expand_as(res1_feat)]
        else:
        ...
        if self.cascade_bu:
            # Added for implementing the bottom up blob_conv 
            # Note that only support min-lvl = 2 max-lvl = 5 or 6 case
            fpn_middle_blobs = []
            fpn_middle_blobs.append(fpn_inner_blobs[-1])
            for i in range(2, self.num_backbone_stages + 1):
                fpn_tmp = self.cascade_strideconv_modules[i - 2](fpn_middle_blobs[0])
                fpn_middle_blobs.insert(0, fpn_tmp)
            for i in range(self.num_backbone_stages):
                fpn_output_blobs.append(
                    self.posthoc_modules[i](fpn_middle_blobs[i])
                )
        else:
            # Original FPN implementation
            for i in range(self.num_backbone_stages):
                fpn_output_blobs.append(
        ...
        if self.P2only:
            ...
            return fpn_output_blobs, conv_body_blobs[-1]
        elif cfg.LESION.SHALLOW_POSITION:
            return fpn_output_blobs, pos_cls_pred, pos_reg_pred
        else:
            # use all levels
...
class topdown_lateral_module(nn.Module):
    ...
    def forward(self, top_blob, lateral_blob):
        ...
        # TODO: check performance interpolate and upsample
        # Interpolate is preferred to replace upsample in torch > 0.4.1
        #td = F.interpolate(top_blob, size=lat.shape[2:], scale_factor=None, mode='nearest')
...
class fpn_rpn_outputs(nn.Module):
    ...
    def _init_weights(self):
        #if cfg.LESION.CONCAT_BEFORE_RPN or cfg.LESION.SUM_BEFORE_RPN:
            #init.normal_(self.FPN_RPN_conv_embedding.weight, std=0.01)
            #init.constant_(self.FPN_RPN_conv_embedding.bias, 0)
    ...
    def detectron_weight_mapping(self):
        ...
        if cfg.LESION.CONCAT_BEFORE_RPN or cfg.LESION.SUM_BEFORE_RPN:
            mapping_to_detectron = {
            #'FPN_RPN_conv_embedding.weight': 'conv_rpn_fpn%d_w' % k_min,
            #'FPN_RPN_conv_embedding.bias': 'conv_rpn_fpn%d_b' % k_min,
    ...
    def forward(self, blobs_in, im_info, roidb=None):
        ...
        for lvl in range(k_min, k_max + 1): #每层过一次FPN
            slvl = str(lvl)
            bl_in = blobs_in[k_max - lvl]  # blobs_in is in reversed order

            #if cfg.LESION.CONCAT_BEFORE_RPN or cfg.LESION.SUM_BEFORE_RPN:
                #fpn_rpn_conv_embedding = F.relu(self.FPN_RPN_conv_embedding(bl_in), inplace=True)
...
def fpn_rpn_losses(**kwargs):
    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        if cfg.DEBUG:
            count += (rpn_labels_int32_fpn==1).data.sum()
            #print('rpn positive num for %s is:' % slvl, ((rpn_labels_int32_fpn==1).data.sum()))
        ...
        BM=1
        if BM:
            # from tools.mask2bbox import one_img_gt_bbox_as_maps
            from mask2bbox import one_img_gt_bbox_as_maps
            gtboxs=kwargs['GTbox']#这里的batch是不分的，全放在一起，所以只能先假设batch=1
            gt_BM=[]
            for gtbox in gtboxs:
                gtBM_one_img=one_img_gt_bbox_as_maps(gtbox)
                gt_BM.append(gtBM_one_img)
            if len(gt_BM)>1:
                gt_BM=torch.cat(gt_BM)
            else:
                gt_BM=gtBM_one_img.unsqueeze(0)
            device=rpn_labels_int32_fpn.device
            gt_BM=torch.nn.functional.interpolate(gt_BM,(h,w))
            gt_BM=gt_BM.to(device)
        ...
        if cfg.RPN.CLS_ACTIVATION == 'softmax':
            ...
            if BM:
                loss_rpn_cls_fpn = F.binary_cross_entropy_with_logits(
                    kwargs['rpn_cls_logits_fpn' + slvl], gt_BM,
                    size_average=True)
            else:
                loss_rpn_cls_fpn = F.binary_cross_entropy_with_logits(
    ...
    if cfg.DEBUG:
        print('rpn positive num is: %d' % count)
...
def fpn_level_info_ResNet26_conv5():
    return FpnLevelInfo(
